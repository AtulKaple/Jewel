...
export default function Home() {
  ...
  const lenis = useLenis(); // Hook to get the Lenis instance
  ...
  useEffect(() => {
    if (!lenis) return; // Wait for Lenis to be available
    ...
    // Refresh ScrollTrigger after the page is loaded
    ScrollTrigger.refresh();

    return () => {
      lenis.off("scroll", onScrollUpdate);
    };
  }, [lenis]); // Dependency on the Lenis instance
